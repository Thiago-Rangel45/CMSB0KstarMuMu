...
# Verifica se o usuario passou a Era
if [ -z "$1" ]; then
...
if [ ! -d "$INPUT_DIR" ]; then
    echo "Erro: A pasta $INPUT_DIR nao existe no EOS."
    exit 1
fi
...
if [ "$NUM_FILES" -eq 0 ]; then
    echo "Erro: Nenhum arquivo encontrado em $INPUT_DIR."
    rm -f "$LIST_FILE"
    exit 1
fi
...
# 3. Trava de seguranca: So apaga se o hadd foi perfeito
if [ $? -eq 0 ]; then
    echo "=============================================="
    echo "SUCESSO no Merge! Salvo em: $OUTPUT_FILE"
    echo "Limpando os $NUM_FILES arquivos originais para liberar cota..."
    
    # xargs le o arquivo TXT e deleta os ROOTs sem travar o terminal
    xargs rm -f < "$LIST_FILE"
    
    if [ $? -eq 0 ]; then
        echo "Limpeza concluida! A pasta $INPUT_DIR esta vazia."
    else
        echo "Aviso: O merge funcionou, mas houve um erro ao apagar alguns arquivos."
    fi
    echo "=============================================="
    ...
    echo "Erro durante a execucao do hadd. Os arquivos originais NAO foram apagados por seguranca."
fi
